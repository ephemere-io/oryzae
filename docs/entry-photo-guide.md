...
（単価は 2026-09-16 に公式の価格表で確認済み。`claude-sonnet-5` は当初 $3 / $15 と書いていたが
これは誤りで、正しくは $2 / $10。2026-09-01 予定の値上げが撤回され $2 / $10 が標準価格になった。）

変更するときは `OCR_MODEL` を差し替える。

board の OCR（`board/infrastructure/ocr/anthropic-ocr.gateway.ts`）も **2026-09-16 から
同じ `claude-sonnet-5`**。あちらは「誤読がそのままスニペットの中身になる」ことを理由に
...
`maxOutputTokens` が抑えるのはトークン数であって文字数ではないため、本文
（`MAX_CONTENT_LENGTH` = 100,000 字）へ流れ込む前にここで抑える。
...
- ~~本文の途中に写真を差し込む表示~~ → **済み**。PC も SP も contentEditable の中に `<img>`
  （DOM との往復は `features/shared/entries/utils/inline-image-codec.ts`、SP は `sp-body-editor.tsx`。
  回り込みは float で文字が写真の横を流れ、SP は選んだ写真を指で掴んで別の文字の位置へ動かせる）。
  保存形式はどちらも `content` の U+FFFC + `effects.inlineImages`。本文の中に居ない写真（旧形式）だけ
  本文の下に並べる。
