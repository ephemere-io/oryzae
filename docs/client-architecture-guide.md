...
| API 通信 | Hono RPC (`hono/client`)。エンドツーエンドの型安全性を実現 |
...
```
apps/client/src/
├── app/                — Next.js App Router。薄いラッパーのみ。API 呼び出し禁止
│   ├── layout.tsx      — ルートレイアウト
│   ├── page.tsx        — トップページ
│   └── {route}/
...
| --- | --- | --- |
| **app/** | ルーティング、レイアウト、features の組み合わせ | API 呼び出しの詳細、状態管理ロジック |
| **features/{feature}/** | 機能固有のUI・ロジック・型の全て | 他の feature の内部実装 |
| **components/ui/** | 再利用可能な汎用 UI（Button, Modal 等） | feature の存在、app のルーティング |
...
- **features/X → features/Y: 禁止** — 機能間の直接依存は許さない。共通ロジックは `lib/` に、共通UIは `components/ui/` に切り出す
- **components/ui → features: 禁止** — 汎用UIが特定機能に依存すると再利用性が破壊される
...
```typescript
// features/entry/hooks/use-entries.ts
export function useEntries() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await client.api.v1.entries.$get();
        if (!res.ok) {
...
```typescript
// features/entry/components/entry-list.tsx
...
- **`as` キャスト禁止** — 型が合わない場合は型ガードを書く
- **`any` 禁止** — `unknown` を使い、型を絞り込む
- **Hono RPC の型推論を活用する** — バックエンドの型がそのままクライアントに伝搬する利点を最大化する
...
import type { AppType } from "@oryzae/server";

export const client = hc<AppType>(process.env.NEXT_PUBLIC_API_URL!);
```

Hono RPC により、リクエスト・レスポンスの型がバックエンドの定義から自動推論される。手動で型を定義する必要はない。
...
// features/entry/hooks/use-create-entry.ts でそのまま使う
