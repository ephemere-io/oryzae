...
重大な指摘があれば GitHub Issue に起票（既存 Issue があればコメント追記）する。
**指摘ゼロなら起票しない** — 毎回ノイズを立てると誰も読まなくなるため。

**ここが AI 監査の主役。** 費用が実行回数だけで決まり PR 数に左右されないため、
上限が読める。週次から月 2 回に落としたのは費用と露出時間の釣り合いによる判断で、
「見つかるまで最大 2 週間」の遅れを負うのは AI にしか見えない種類の問題だけ
