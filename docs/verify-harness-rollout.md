...
- **全コンポーネント移植（A）**: 全 client feature コンポーネントをカバーし、毎 PR で自動検証・可視化する全面適用の実行計画は [`plans/verify-harness-full-migration.md`](./plans/verify-harness-full-migration.md)。**着手は Issue #363（device-first 再編）が main にマージされてから**（feature の置き場が移動するため）。
