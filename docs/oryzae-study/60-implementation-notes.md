...
## 12. 泡は発酵が完了すると止まる（仕様どおり）

`00-overview.md`「完了すると泡が止み、封が浮かぶ」、`21-3d-parameters.md`
「泡の速度 …（完了時 0＝静止）」、原案も `speedScale = done ? 0 : ...` で同じ。

つまり**手紙が届いている瓶では泡が動かないのが正しい**。発酵中は上がる
（readiness 0.9 で 4 秒あけて撮った 2 枚を比べ、泡の位置が変わることを確認済み）。

もし完了後も泡を動かしたいなら、それは原案と仕様の両方を変える判断になる。
