...
### 画面の高さは「ツールバーを除いた表示領域」（SP）

紙は画面の下に浮かせるので、**下のツールバーに隠れない**ことが要る。ブラウザは 2 通りある。

1. **自分でよけるブラウザ**（Safari など）: ツールバーぶんを引いた高さを `100svh` で教えてくれる。
   認証画面の高さは `100svh` を下限にし、**`<body>` いっぱいには伸ばさない**（`flex-1` を付けない）。
   `<body>` は `h-full` で、iOS ではツールバーの裏まで含む大きい方のビューポートになる。そこへ
   伸ばすと下端の紙がツールバーの裏に隠れる（実機レビュー 1 回目）
2. **重ねて描くブラウザ**（Dia などのアプリ内ブラウザ）: Web ビューを画面いっぱいに置き、
   ツールバーをその上に重ねる。`svh` も `dvh` も `env(safe-area-inset-bottom)` も画面の高さのままで、
   **ページからは隠れていることが分からない**（実機レビュー 2 回目）

2 は CSS だけでは避けられないので、`entrance/viewport.ts` が**ビューポートが画面ぴったりを
覆っているか**を見て判定し、隠れているぶんを画面の高さの割合で見積もって紙の下に足す
（実測: iPhone 15 Pro の Dia で 852pt 中 76pt = 8.9%）。測るのは開いたときの 1 回だけ —
スクロールのたびに測り直すと、ツールバーが畳まれるたびに紙が跳ねる。
