...
interface StudyState {
  ...
  board: { dateKey: string; viewType: 'daily' | 'weekly'; cards: StudyBoardCard[] };
}
...
| `board.cards` | `GET /api/v1/board?dateKey&viewType&tzOffset`（`use-board.ts`） | `normalizeBoardCards` を通す |
