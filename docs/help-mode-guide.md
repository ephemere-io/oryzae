...
面の頭の 1 枚は**いま開いている画面**の説明。画面の部品が並ぶ（`topics.ts` の
`screenParts`）。書斎は**手描きの見取り図**（`help-study-map.tsx` — 3D の構図を線で写し、
板が奥、机の左に瓶、真ん中に手帳の束、右奥に書庫、手前右に鉛筆。線は feTurbulence で
わずかに揺らし、字は揺らさない）。札の字は部屋に貼ってある注釈そのもの（`study.label_*` —
JAR / ENTRIES / BOARD / ARCHIVE / NEW）。書斎の物を指す話題の題も「手帳（ENTRIES）」のように
語と注釈を両方持つ（`docs/glossary.md`）。他の画面は部品の札を並べるだけ。画面の中で瓶に
