...
| 面 | 右の面（`features/pc/help/components/help-sidebar.tsx`）。`<main>` の隣に立ち、本文はそのぶん詰まる（被らない）。幅 0 ⇄ 面の幅で出入りする（160ms）。既定の幅・縁は発酵の面と同じ、地は紙、縁に左へ落ちるごく薄い影。**左の縁を掴んで幅を変えられる**（280–560px、憶える。2 度押しで既定に戻る。キーボードでは ←/→ で 16px ずつ、Home/End で最小・最大。掴み手は縁をまたぐ 14px で見えず、触れたときだけ縁に細い線）。面が立って書斎が横に狭くなると、書斎は机が丸ごと入るぶん引く（`zoomForAspect`） | 下から出るシート（`features/sp/help/components/sp-help-sheet.tsx`） |
| 閉じる | 面の ×／`Esc`（検索欄に文があるときは先に文が消え、空でもう一度押すと閉じる。日本語入力の変換中の `Esc`、面の外で字を打っている最中の `Esc`、手前にメニューやダイアログが開いているときの `Esc` は触らない）／`?`／右上の「?」をもう一度。面の中に focus があったまま閉じたら focus は「?」へ戻る | 面の ×／背景 |
...
### 画面の 1 枚（`help-screen-card.tsx`）

面の頭の 1 枚は**いま開いている画面**の説明。画面ごとに**手描きの見取り図**がある
（`help-screen-map.tsx`。部品は `topics.ts` の `screenParts` と 1:1）— 書斎は 3D の構図を線で
写し（板が奥、机の左に瓶、真ん中に手帳の束、右奥に書庫、手前右に鉛筆）、瓶は大きな瓶の中の
問いの円と手紙と右上の「問いの変遷」、書く画面は縦書きの行と左上のチップと下の真ん中のパレット、
ボードは板のカード、一覧は今月と過去の月の行、問いの変遷は左の縦の年表と右のカード。線は feTurbulence でわずかに
揺らし、字は揺らさない。札の字はその画面に出ている名前 — 書斎は部屋に貼ってある注釈
（`study.label_*` — JAR / ENTRIES / BOARD / ARCHIVE / NEW）、他は `help.map.<画面>.<部品>`
（「問いを紐づける」「漬け込む」…）。書斎の物を指す話題の題も「手帳（ENTRIES）」のように
語と注釈を両方持つ（`docs/glossary.md`）。図の無い画面（アカウント）は本文だけ。画面の中で瓶に
触れると瓶の札が灯って説明が開き、面の中で札に触れても同じ。札を押すと留まる（留めは画面ごと —
画面を移ると外れる。`HelpScreenCard` は画面の id を key に作り直す）。1 枚そのものは
画面を移るまで変わらない — 前の版は触れているものをそのまま映していて、「何もしないのに勝手に
変わって注意を奪われる」と言われた。部品でないもの（右上の「?」など）に触れても何も起きない。

**少し止まってから映す** — 見取り図の物が灯るのは 40ms（ほぼすぐ）、説明文が差し替わるのはさらに
100ms 後（`TEXT_SETTLE_MS`。面の中の触れと留めは待たない）。通り過ぎた物では変えない —
ポインタが物をまたぐたびに 1 枚が入れ替わると、何もしていないのに勝手に変わって見えて注意を
奪う（オーナーの同僚の最初の感想）。一律 450ms 待っていた版は「反応が遅い」と言われたので、
灯りと文を分けた。一度映したら次に止まるまでそのまま（隙間に出ても消さない）。消えるのは、面の
中に入ったとき・画面を移ったとき・閉じたとき。

線画（88px、アクセント色）・題・一言・本文。触れているものが変わると 1 枚ごと入れ替わる
（`help-fade`、120ms）。動きが「変わった」ことを言う。何にも触れていなければ、
いま開いている画面（`topicForScreen`）。**面の中に入ったら「触れていない」に戻る** —
面の隣の物（瓶の画面の「問いの変遷」）を横切った直後に面へ入ると、その物の説明が
面の中に居座っていた。

### チュートリアル（`help-tutorial.tsx`）

五歩: 問いを立てる → エントリーを書く → 問いを紐づける → 瓶に漬けて待つ → 手紙を読む。
旗は `hasQuestion`・`hasEntry`・`hasLinkedQuestion`・`hasPickled`・`hasReadLetter`。
アコーディオンで、済んでいなければ開き、五歩とも済んだ瞬間に閉じて見出しだけが残る（開き直せる）。
**面の中の位置は変わらない**（検索欄 → 画面の 1 枚 → ここ → 一覧）。検索欄もいつも居る。

**一個ずつ済ませていく。** 済んだ歩は番号が印に変わり、いまの歩は番号が脈打ち、まだの歩は
薄い。進み具合は憶えない — サーバーの旗そのもの（`/api/v1/users/me` の `hasQuestion`・
`hasLinkedQuestion`・`hasPickled`。`useHelpProgress`）。「問いを立てた」「問いを結んで
書いた」「漬け込んだ」が、どれも一度でもやったか。何かを成し遂げたら questions / entries の
hook が `lib/activity` の合図（window のイベント、種類だけ）を出し、面はそれを聞いて旗を
取り直す。ドメインをまたぐ import は無い。

開いている間だけ html に `data-tutorial-step="<歩>"` が付き、画面の中の
相手 — 瓶の「問いを追加する」、エディタの「問いを紐づける」チップ、パレットの「漬け込む」
（`data-tutorial="<歩>"`）— が番号と同じ脈で灯る（`globals.css`）。面が閉じていれば灯らない
（案内を追っていない人の画面で、ボタンがいつまでも脈打つのは邪魔なだけ）。①を押して瓶の
画面に来たときは、空の瓶を大きく見せる（`JarView` の `emphasizeEmpty`）。五歩とも済めば、
チュートリアルは閉じて見出しだけになる。
...
3. どちらでも決まらなければ、何も出さない（画面の話題のまま）。**名前は外に送らない** — 押せるものの名前は aria-label や textContent から拾うので、利用者が書いた文（瓶の画面の問いの本文など）が混ざりうる。Jev に訊くのは検索欄の文だけ
...
- 選択肢（話題の題と一言）はクライアントが毎回送る。文面の正は i18n にあり、サーバーで
  二重に持たない
- 送るのは検索欄の文・画面の話題（パスではなく `topicForScreen` の id）・言語だけ。触れている部品の名前は送らない。**エントリーの本文は送らない。** サーバーは
  問いの本文をログにも載せない
- 同じ問いは憶えていて、二度目は訊かない

## 初めての人

サインアップ直後の着地は書斎（`/`）。エントリー画面に着くと、何の部屋か分からないまま
書かされる。書斎で「ようこそ」→「始めてみよう」→ 三歩の①、と続く。
...
押すと晴れ、面はそのまま残って全部が元の濃さに戻る（500ms）。押した時点で見たことにして記録する（面は開いたまま画面を移れるので、閉じるまで待つと次の読み込みでまた「ようこそ」が出る）。「?」の脈打ちは初めて閉じたときのまま。設定で切っている人には自動で開かず、見たことにするだけ（開くと設定が有効に戻ってしまう）。「始めてみよう」の後にもう 1 段の
