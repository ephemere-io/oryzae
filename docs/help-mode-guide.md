...
いまの歩がある間、面は三歩を頭に置き、検索欄を出さない（始めたばかりの人は検索より、次に
何をするかが要る）。開いている間だけ html に `data-tutorial-step="<歩>"` が付き、画面の中の
...
画面に来たときは、空の瓶を大きく見せる（`JarView` の `emphasizeEmpty`）。三歩とも済めば、
面はいつもの形（1 枚が頭、検索欄あり）に戻り、三歩は印だけになる。
