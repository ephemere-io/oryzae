...
| 面 | 右の面（`features/pc/help/components/help-sidebar.tsx`）。`<main>` の隣に立ち、本文はそのぶん詰まる（被らない）。幅 0 ⇄ 面の幅で出入りする（160ms）。既定の幅・縁は発酵の面と同じ、地は紙、縁に左へ落ちるごく薄い影。**左の縁を掴んで幅を変えられる**（280–560px、憶える。2 度押しで既定に戻る。掴み手は縁をまたぐ 14px で見えず、触れたときだけ縁に細い線）。面が立って書斎が横に狭くなると、書斎は机が丸ごと入るぶん引く（`zoomForAspect`） | 下から出るシート（`features/sp/help/components/sp-help-sheet.tsx`） |
...
2 つだけ — 使い方は面（PC は右、SP は下）にあること、「始めてみよう」。この間、面の中でも
三歩だけを明るくし、生きている 1 枚と一覧は薄くする（`HelpPanel` の `spotlight`、`welcome` を
そのまま渡す）— 面以外が沈んでいる画面で、面の中でも見る場所を 1 つにする。ボタンか沈みを
押すと晴れ、面はそのまま残って全部が元の濃さに戻る（500ms）。「始めてみよう」の後にもう 1 段の
強調を足さない（情報量が多くて気持ち悪くなる）。面を閉じても晴れる。
