...
**書斎のメモ帳はヘルプの入口ではない。** 一度メモ帳を押すと面が開閉する形にしたが、
物を押してモードが切り替わるのは戻り道が読めない（「メモというよりヘルプモードという
体験」）。物は物として、押すと公開サイトのヘルプ（`/support`）を新しいタブで開く。
...
| 面 | 右の面（`features/pc/help/components/help-sidebar.tsx`）。`<main>` の隣に立ち、本文はそのぶん詰まる（被らない）。既定の幅・地・縁は発酵の面と同じ。**左の縁を掴んで幅を変えられる**（280–560px、憶える。2 度押しで既定に戻る） | 下から出るシート（`features/sp/help/components/sp-help-sheet.tsx`） |
...
上から **検索欄 → 生きている 1 枚 → 話題の一覧**。それだけ（`help-panel.tsx`）。
...
- 一覧に節の見出し（「はじめに」「書斎のもの」）を付けない。行の間の空きで束が分かり、
  行の線画で何の話かが分かる。上から読めば一周する順に並んでいる
- 初めての人に「ようこそ」の文を出さない。面が開いた状態で始まる、それで足りる
...
線画（88px）・題・一言・本文。触れているものが変わると 1 枚ごと入れ替わる
（`help-fade`、180ms）。動きが「変わった」ことを言う。何にも触れていなければ、
いま開いている画面（`topicForScreen`）。

### 一覧

話題 15 件を 4 つの束に分けて並べる（束の間は空きだけ）。行は線画（30px）・題・一言。
押すと本文と「開く →」が出る（外へ出る話題は ↗）。
...
| 書斎の 3D の的（`topicForStudyLabel`） | jar / notebook / board / archive / write / support（メモ帳）。何にも触れていなければ画面の話題 |
| 書斎の左下のアバター | account |
...
false なら**面を開いた状態で始める**。初めて閉じたとき、右上の「?」が 3 回脈打ち、隣に
「ヘルプ」の字が数秒だけ出る — 言葉で「ここから開けます」と説明する代わりに、動きと位置で
居場所を教える。同時に `PATCH /api/v1/users/me/onboarding` で記録し、以後は自動で開かない。
...
| 面の中身 | `features/shared/help/components/help-panel.tsx`（+ `help-live-card` / `help-topic-card` / `help-illustrations`） |
