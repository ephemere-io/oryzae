...
`cron-cost-alert.ts` が JST 10:00 に前日ぶんを流す。狙いは
**「金額を報告する」ではなく「このままで良いかを判断できるようにする」**。

| ブロック | 出すもの | なぜ |
|---|---|---|
| 実請求額 / 今月の累計 / 月末の見込み | 実額（org 全体・UTC 日）と前日比、月初からの累計、平均の単純延長 | 単日の金額だけでは多いのか少ないのか判断できない |
| 実額の内訳（モデル別） | モデル別の実額 + 発酵の推定との突き合わせ + Console リンク | 「何にかかっているか」と「推定が合っているか」を1か所で読む |
| 発酵 / 利用者 / 1 発酵あたり | 件数・成功失敗・人数・1件あたりの推定コストとトークン | 金額が動いたとき「使われ方が変わったのか、単価が変わったのか」を切り分ける |
| トークン | 入力・出力の合計に**何のトークンか**と**単価**を添える | 「in 5,336 / out 7,069」だけでは誰の何か分からない。単価があればこの欄で検算できる |
| ユーザー別 | 推定の内訳 + なぜここだけ推定かの注記 | 実額と並ぶと、注記の無い推定値は「意味の分からない2つ目の金額」になる |
| 推定の計算根拠 | 計算式（**条件付き**） | 実額とズレたとき・実額が取れないときだけ。平常時はトークン欄の再掲 |
| 要確認 | 閾値超過・推定と実額の乖離・トークン未記録・集計打ち切り（**条件付き**） | — |

原則:

- **平常時に出す数字は、判断に使うものだけにする。** 「未計上 0 件」を毎日出すと、
  本当に 1 件出た日に読み飛ばす。異常系は「要確認」に寄せ、何も無ければ欄ごと出さない。
  発酵 0 件の日も同様に、0 の再掲になる欄（利用者・1件あたり・トークン・ユーザー別）は出さない。
