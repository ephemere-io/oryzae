...
| 請求額（Anthropic の org 全体の実額） | 「合計: 金額（前日比）」の配下に用途別の内訳を `├` `└` でぶら下げ、混ざりうる旨の注記と確認先（管理画面・Console）を添える | 「$0.3263 が何で、$0.2939 が何か」を親子の字形で読む。スコープ（org 全体）は欄の名前に置き、数字ごとに注記しない。空白のインデントは Discord が削るので罫線で階層を出す |
