...
const inlineImageSchema = z.object({
   ...
   * 写真は常に独立した行の中央に置くので、傾けても本文の流れは崩れない。
   */
  rotation: z.number().optional(),
});

/**
 * 配置は **独立した行の中央** に固定してある（Notion / Medium と同じ）。
 *
 * 以前は行内 / ブロック / 回り込みと寄せ 3 種を選ばせていたが、「項目が多く、全部試さないと
 * 意味が分からない」という指摘を受けて畳んだ。写真を行頭に小さく置きたい場面が実際には
 * ほぼ無く、選択肢の数だけが負担になっていた。保存形式からも外してあるので、
 * 復活させるときは schema から足し直すことになる。
 */
