...
このセッションには 1 回あたりの USD 上限がある。上限に達すると **作業の途中で打ち切られ、
編集した内容ごと捨てられる**。「途中まで直した」は 0 点で、費用だけが減る。

実測（2026-09-06）: 1 ターンあたり約 $0.02。上限 $0.60 なら **30 ターン程度で打ち切られる**。
したがって:

- 最初に広く読み漁らない。ファイル一覧は 1 回で取り、怪しい順に絞って読む
- 「もう少し読めば分かるかも」で粘らない。20 ターンを目安に、直すか諦めるかを決める
