...
/**
 * 「最近書いているか」の判定窓（日数）。**30 日 = MAU（Monthly Active User）**。
 *
 * 日数そのものはツールチップに埋め込んであるので表示は自動で追従するが、
 * **「MAU」という呼称は 30 日であることが前提**。この値を変えるなら
 * `components/user-table.tsx` のツールチップから MAU の表記も外すこと。
 */
