/**
 * Oryzae ブランド定数（admin 側）。LP（ランディングページ）と同じ正式ロゴ・配色を共有する。
 * - SVG ロゴ: client の P3_appicon_cream.svg と同一
 * - 用途: ファビコン、Apple touch icon
 *
 * admin は社内向けなので OGP は不要。client 側の lib/brand.ts と意図的に重複させているのは、
 * apps/admin が apps/client に依存しないようにするため（モノレポのアプリ独立性）。
 */

export const BRAND_NAME = 'Oryzae Admin';
