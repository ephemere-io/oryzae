 ...
 * これ以上は引けない倍率。
 *
 * 20% だったものを 10% に下げた。ボードが 1 人 1 枚になって全期間のカードが 1 つの板に
 * 乗るようになると、20% では**「全体を見る」でも全部が収まらない**（実測: 7 枚で
 * 0.18 倍が要るのに 0.2 で頭打ち）。「もうちょっと引けても良い」という実機の指摘も同じ所を
 * 指している。下げても引き切った先は書斎（`OVERZOOM_ARM_SCALE`）なので、行き止まりにはならない。
 */
 ...
 * 引きの余りを知らせ始める倍率。
 *
 * **最小に着いてからでは遅い。** 板は等倍で開き、最小までにはつまみ 3〜4 回ぶんの
