...
/**
 * 重なりの順（z-index）。**エディタの fixed 層の中で競うもの**をここに並べる。
 *
 * パレットは書いている間ずっと浮いているもの、ポップオーバー（設定パネル）は押して開いた
 * 一時的な面。**開いた面が常駐の道具の下に潜ってはいけない。** 以前はパネルが z-70・
 * パレットが z-1600 で、パレットを右上へ動かしているとパネルの下半分が塞がれ、
 * スクロールも「このエントリーを消す」も届かなかった。
 *
 * 説明の吹き出し（HelpHint）は body 直下に出るので、エディタの層ごと越える。
 * 確認のモーダルは各所で `z-[2000]` を使っている（この表のどれよりも上）。
 */
export const LAYER = {
  palette: 1600,
  popover: 1700,
