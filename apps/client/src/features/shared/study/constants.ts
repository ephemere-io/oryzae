...
export const HOME_ZOOM = {
  ...
  wheelStep: 0.0012,
  /**
   * 目標へ寄せる時定数（ms）。値は `1 - exp(-dt / followMs)` で毎フレーム寄る（フレームレートに依らない）。
   * 指に 1:1 で追わせると動きが単調になり（実機レビュー: 弧を描く動きが消えた）、フレーム係数 0.12
   * （≒130ms）では指より遅れて滑った（実機レビュー: 即答性が低い・余計な動き）。丸さを残して遅れを削る。
   */
  followMs: 70,
} as const;
...
export const ARRIVAL = {
   ...
   * 水平の距離の倍率（1 より大きいと手前）。
   *
   * **溶けているあいだも動いていること**が要る。扉の側は歩いて着き、こちらはまだ
   * 寄っている最中 — その重なりが「続いている」を作る。1.1 では動きがほとんど見えず、
   * 静止画から静止画へ切り替わったように見えた（実機レビュー「まだ一瞬切れる」）。
   * かといって派手に寄ると「入ってきた」ではなく「飛んできた」になる。
   */
  distanceScale: 1.34,
