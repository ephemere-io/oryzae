...
/**
 * サブ画面の下端に覗く「部屋への入口」の寸法。
 *
 * **全幅にしない。** ボードと瓶は左下に倍率、右下にミニマップを置いているので、
 * 全幅の帯はその上に乗る。中央だけを使えば、どの画面の操作とも取り合わない。
 */
export const DOORWAY = {
  /** 触っていないときの高さ（px）。名前が 1 行入る最小限。 */
  restHeight: 56,
  /** 同じく幅。画面が狭ければ 100% に丸まる。 */
  restWidth: '460px',
  /** 帯の上の角丸（px）。開ききると 0 になる。 */
  restRadius: 14,
  /** 手を離してから元の高さへ戻る時間（ms）。引いている最中は掛けない。 */
  settleMs: 260,
  /** 名前の大きさ（px）。9px では下端で読めなかった。 */
  labelSize: 12,
  /** 憶えた部屋が無いときの地（`LIGHT_PALETTE.solid`）。 */
  ground: '#FDFCF9',
} as const;
