...
/**
 * ホームで、押せる物の無いところを指（マウス）で引くと机の上を平行に動く（実機レビュー: 何も無いところを
 * 引いても何も起きない）。**回り込みはさせない**（`HOME_ZOOM` と同じ理由で、構図を保つ）。
 * 動ける範囲は、どれかの物を画面の真ん中に持ってこられるところまで（`clampPan`、配置表の物の位置から決まる）。
 */
export const HOME_PAN = {
  /** 押したのか引いたのかを分ける指の移動（CSS px）。瓶の地図の円と同じ。 */
  tapSlop: 8,
} as const;

/**
 * 「引き切ったところから、もう一段引く」で書斎へ戻るときの手応え。
 *
 * **1 回のはじきでは着かない量**にしてある。トラックパッドを 1 度弾いただけで部屋が
 * 変わると、意図しない離脱になる。逆に重すぎると引けることに気づけないので、
 * 「気づいてから、そのまま引き続ければ着く」あたりを狙っている。
 */
export const PULL_BACK = {
  /**
   * はみ出した引き 1 に対して進む割合。
   *
   * はみ出しは 1 回の操作あたり `1 - exp(-Δ × 感度)` で積み上がり、**倍率の対数距離に
   * ほぼ等しい**。0.45 だと、最小まで引き切ってから **さらに 7 倍ぶん引き続ける**と着く。
   * 0.9（＝3 倍ぶん）で試したところ、トラックパッドを 1 度弾いただけで部屋が変わり、
   * 「引いてみる」より先に離脱してしまった。
   */
  gain: 0.45,
  /**
   * 指のつまみの進み。つまみ 1 回の距離は指の開き幅で頭打ちになり（最小まで引いたあと 1 回で
   * 対数距離 1 前後）、ホイールと同じ 0.45 では**戻れそうで戻れない**（レビュー）。はっきり引けば 1 回で
   * 着く値にする（1.3 では、戻したくない軽いつまみでも着いた。レビュー）。
   */
  pinchGain: 1,
  /**
   * 指を離した時点でここまで進んでいれば、戻らずに書斎へ着かせる（確実に戻れるように）。
   * 半分では戻したくないときにも戻ってしまった（レビュー）。はっきり引いたときだけ着く量に。
   */
  commitOnRelease: 0.7,
  /** 手を止めてから戻し始めるまで（ms）。飛び飛びに届くイベントの間で点滅させない。 */
  holdMs: 220,
  /** 戻る速さ（1 フレームあたり）。約 0.5 秒で消える。 */
  decayPerFrame: 0.035,
  /** 滲み始めの部屋の寄り。引き切ると 1 へ戻る。 */
  startScale: 1.08,
  /**
   * 滲み始めのぼけ（px）。引き切ると 0 へ。
   *
   * **強くかけない。** 書斎は細い線で出来ているので、ぼかすと部屋だと分からなくなり、
   * ただ画面が曇ったようにしか見えない（10px で実際にそうなった）。
   */
  startBlurPx: 3,
} as const;
