...
export interface StudyLayout {
  ...
  /**
   * 壁のメモ（ヘルプ・お問い合わせ・Docs への導線）の中心と、貼る面。null は置かない。
   *
   * 他の物と同じ線画の物（`scene.ts` の `buildMemo`、寸法は `scene/memo.ts`）。
...
export const PC_LAYOUT: StudyLayout = {
  ...
  pillOffsets: null,
  /**
   * ボードの左隣の壁（オーナーの依頼）。板は x = 1.7 ± 4 なので左端は -2.3。紙の幅は
   * 2.0（`scene/memo.ts`）なので、中心を -4.3 に置くと板との間に 1.0 空く。瓶（x = -4.2）
...
export const SP_LAYOUT: StudyLayout = {
   ...
   */
  memo: { position: vec3(-2.3, -1.19, 3.0), surface: 'desk' },
