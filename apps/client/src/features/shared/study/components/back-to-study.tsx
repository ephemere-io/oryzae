...
/**
 * SP のサブ画面の上端の中央に垂れ下がる「書斎へ戻る」のタブ。
 *
 * **引く動作（`PullBackToStudy`）と同じことを、押しても出来るようにするための双子。**
 * 引きは覚えなくてよい代わりに、そこに在ることが見えない。キャンバスを持たない画面
 * （エントリー）には引きの軸すら無い。だから見える出口も 1 つ要る。
 *
 * **PC はこれを使わない。** PC の出口は各画面のヘッダーの先頭の「‹ 書斎」
 * （`components/ui/back-link`）。中央に垂れるタブは、どの画面にも属さない札が
 * 真ん中に貼られ続けているようで「アプリの期待値として違和感がある」と言われた。
 *
 * ### ここまでの経緯
 *
 ...
 * ### 画面の側がすること
 *
 * SP は題を中央に置くので、タブの高さぶん下がる（`--study-exit-band`）。
 *
