...
/** 1 件あたりの厚み。 */
const THICKNESS_PER_ENTRY = 0.01;

/**
 * 表紙だけの最小の厚み。
 *
 * **0.1 から上げた。** 積みの中段・下段が薄すぎて押しにくい、と実機レビューで
 * 報告された。当たりの高さは厚みそのものなので、薄い冊は帯が数 px しか無く、
 * 天板の縁と見分けもつかなかった。物として厚くすることで、見た目と当たりの
 * 両方が一度に直る（当たりだけ広げると、見えている冊の外を押せることになる）。
 *
 * **上げるのは下限だけ。** 1 件あたりの伸びは逆に 0.012 → 0.01 へ落とした。同じ比で
 * 上げると、書いた月の冊が板のように分厚くなり、手帳ではなく発泡スチロールの塊に見える
 * （0.016 で試して実際そうなった）。押しにくいのは薄い冊なので、効かせるべきは下限。
 */
