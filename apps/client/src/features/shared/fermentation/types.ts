...
/**
 * 発酵瓶アニメーション用の集計 readiness（Issue #278）。
 *
 * PR #291 以降 readiness は問い単位なので、サーバーは active な問い 1 件あたり [0, 1] の
 * readiness を計算した合計（`totalReadiness`, [0, 3]）を返す。
 * 次回 eligible 時刻は意図的に返さない（ユーザーには「いつ来るか分からない」体験を維持する）。
 */
