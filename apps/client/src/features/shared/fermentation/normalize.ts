...
/**
 * 集計 readiness レスポンスを正規化する（Issue #278）。
 * 形状が不正なら null（＝瓶の演出は readiness 0 のまま）。壊れた値でアニメーションを
 * 揺らすより、静かな瓶に倒す方が安全。
 */
export function normalizeReadiness(input: unknown): FermentationReadiness | null {
  if (!isObject(input)) return null;
  const { totalReadiness, questionCount, language } = input;
  if (typeof totalReadiness !== 'number' || typeof questionCount !== 'number') return null;
  if (language !== 'ja' && language !== 'en') return null;
  return { totalReadiness, questionCount, language };
}
