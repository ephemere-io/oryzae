  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <StatCard label={t('label.streak')} value={`${stats.streak}${t('value.streak_days')}`} />
        <StatCard label={t('label.total_entries')} value={stats.totalEntries} />
        <StatCard label={t('label.total_chars')} value={stats.totalChars} />
        <StatCard label={t('label.total_fermentations')} value={stats.totalFermentations} />
        <StatCard label={t('label.weekly_chars')} value={stats.weeklyChars} />
        <StatCard label={t('label.monthly_chars')} value={stats.monthlyChars} />
      </div>
            ...
            {stats.entriesByQuestion.map((q) => (
              <div key={q.questionId} className="flex items-center justify-between text-sm">
                <span className="truncate pr-4" style={{ color: 'var(--fg)' }}>
                  {q.questionText || t('label.untitled')}
                </span>
