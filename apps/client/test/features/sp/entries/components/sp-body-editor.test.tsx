...
function setup(value: string, images: { storagePath: string; signedUrl: string }[]) {
...
describe('SpBodyEditor（文のブロックと写真のブロックの列）', () => {
  ...
  it('写真 1 枚なら textarea 2 つの間に写真が入る', () => {
    setup(`前${P}後`, [{ storagePath: 'p/1.jpg', signedUrl: 'https://signed/1' }]);
  ...
  it('文を書き換えると、写真の位置を保った本文で onChange する', () => {
    const { onChange } = setup(`前${P}後`, [{ storagePath: 'p/1.jpg', signedUrl: 'x' }]);
  ...
  it('写真の直後の文の先頭で BackSpace すると写真を抜く（Notion と同じ）', () => {
    const { onRemoveImage, onChange } = setup(`前${P}後`, [
      { storagePath: 'p/1.jpg', signedUrl: 'x' },
    ]);
  ...
  it('文の途中の BackSpace は普通の削除（写真を抜かない）', () => {
    const { onRemoveImage } = setup(`前${P}後`, [{ storagePath: 'p/1.jpg', signedUrl: 'x' }]);
    const second = segment(1);
    second.focus();
  ...
  it('署名できなかった写真は枠だけ残す（index がずれない）', () => {
    setup(`前${P}後`, [{ storagePath: 'p/1.jpg', signedUrl: '' }]);
