...
import { fetchSentryIssues } from '../../infrastructure/sentry-api.js';
...
/**
 * 連携ツールごとに、このデプロイが読む環境変数。Tools 画面の「設定」列の正。
 *
 * 値は返さない（有無だけ）。**この admin のデプロイ**の状態であって、client
 * （oryzae-client）は別の Vercel プロジェクトなので見えない。画面にもそう書く。
 * 変数を足したら .env.example と docs/observability-guide.md の表も直すこと。
 */
const TOOL_ENV: Record<string, readonly string[]> = {
  sentry: [
    'SENTRY_DSN',
  ...
  posthog: ['NEXT_PUBLIC_POSTHOG_KEY', 'POSTHOG_PERSONAL_API_KEY'],
  anthropic: [
    'ANTHROPIC_ADMIN_KEY',
...
function toolEnvPresence(): Record<string, Array<{ name: string; set: boolean }>> {
  const out: Record<string, Array<{ name: string; set: boolean }>> = {};
  for (const [tool, names] of Object.entries(TOOL_ENV)) {
    out[tool] = names.map((name) => ({ name, set: Boolean(process.env[name]) }));
  }
  return out;
}
...
export const adminTools = new Hono<Env>()
  .get('/summary', async (c) => {
    const [sentry, monthlyActual, vercelDeploy, upstashKeys, resendStats] = await Promise.all([
      fetchSentryIssues(),
    ...
    return c.json({
      env: toolEnvPresence(),
      sentry: {
        // 取得できなかったときは null。0 と潰すと「エラー無し」と誤読させる。
        status: sentry.kind,
        unresolvedCount: sentry.kind === 'ok' ? sentry.issues.length : null,
      },
      anthropic: {
  ...
  .get('/spend', async (c) => {
    ...
    const [actual, rowsResult] = await Promise.all([
      ...
      fetchFermentationCostRows(supabase, {
        ...
        console.error('[admin-tools] spend query failed', { error: message });
        return null;
      }),
    ]);
