...
/** テスト配信済みの下書き。本番送信はこれが前提なので、送信系のテストはこちらを使う。 */
function testedDraft(overrides?: { subject?: string; bodyMarkdown?: string }) {
  return draft(overrides).withTestSent(at('2026-09-01T12:00:00.000Z'));
}
...
describe('Newsletter.withContent', () => {
  ...
  it('送信中・送信済みは編集できない（届いた文面と食い違うため）', () => {
    const sending = testedDraft().withSendingStarted(3);
...
describe('Newsletter.withSendingStarted', () => {
  it('宛先数を記録して sending にする', () => {
    const started = testedDraft().withSendingStarted(42, at('2026-09-03T00:00:00.000Z'));
  ...
  it('宛先 0 名では始めない（誰にも届いていない配信が sent で残るため）', () => {
    const result = testedDraft().withSendingStarted(0);
    expect(result.success).toBe(false);
    if (!result.success) expect(result.error.kind).toBe('not-sendable');
  });

  it('送信中の再送信を弾く（二重送信の防波堤）', () => {
    const started = testedDraft().withSendingStarted(3);
    expect(started.success).toBe(true);
    if (!started.success) return;
  ...
  it('直前の失敗理由は送信開始でクリアされる', () => {
    const aborted = testedDraft().withSendAborted('前回の失敗');
...
describe('テスト配信ゲート', () => {
  // 取り消せない操作なので、実際に届く形を一度も見ないまま撃てないようにする。
  it('テスト配信していない下書きは送信を始められない', () => {
    const result = draft().withSendingStarted(100);

    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.kind).toBe('not-tested');
      expect(result.error.message).toContain('テスト配信');
    }
  });

  it('テスト配信済みなら送信を始められる', () => {
    expect(testedDraft().withSendingStarted(100).success).toBe(true);
  });

  // テストしたあと書き換えて送れると、ゲートが形だけになる。
  it('本文を書き換えるとテスト配信の印が消える', () => {
    const updated = testedDraft().withContent({
      subject: '今月の更新',
      bodyMarkdown: '書き換えた本文',
    });

    expect(updated.success).toBe(true);
    if (!updated.success) return;
    expect(updated.value.testSentAt).toBeNull();
    expect(updated.value.withSendingStarted(100).success).toBe(false);
  });

  it('件名を書き換えてもテスト配信の印が消える', () => {
    const updated = testedDraft().withContent({
      subject: '差し替えた件名',
      bodyMarkdown: '本文です。',
    });
    ...
    expect(updated.success).toBe(true);
    if (updated.success) expect(updated.value.testSentAt).toBeNull();
  });

  // 同じ内容で保存し直しただけで消えると、触るたびにテストし直しになる。
  it('中身が変わらない保存では印を消さない', () => {
    const tested = testedDraft();
    const updated = tested.withContent({
      subject: tested.subject,
      bodyMarkdown: tested.bodyMarkdown,
    });

    expect(updated.success).toBe(true);
    if (updated.success) expect(updated.value.testSentAt).toBe(tested.testSentAt);
  });

  it('送信が中断されてもテスト配信の印は残る（そのまま送り直せる）', () => {
    const started = testedDraft().withSendingStarted(100);
    if (!started.success) throw new Error('unreachable');

    const aborted = started.value.withSendAborted('Resend API 500');

    expect(aborted.testSentAt).not.toBeNull();
    expect(aborted.withSendingStarted(100).success).toBe(true);
  });
});

describe('Newsletter.withSendCompleted / withSendAborted', () => {
  it('送信結果を記録して sent にする（sentAt が入る）', () => {
    const started = testedDraft().withSendingStarted(10);
    if (!started.success) throw new Error('unreachable');
  ...
  it('全員失敗しても sent にする（実行した事実は残す）', () => {
    const started = testedDraft().withSendingStarted(5);
    if (!started.success) throw new Error('unreachable');
